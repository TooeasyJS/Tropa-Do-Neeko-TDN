...
class TrustUserSelect(discord.ui.UserSelect):
    def __init__(self, canal: discord.VoiceChannel, action: str):
        ph = 'Selecione quem confiar...' if action == 'trust' else 'Selecione quem desconfiar...'
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.action == 'trust':
            ...
            await interaction.response.send_message(
                f'🤝 **{member.display_name}** agora pode entrar mesmo com o canal bloqueado!', ephemeral=True
            )
        else:
...
class TrustView(discord.ui.View):
    def __init__(self, canal, action):
...
class BlockUserSelect(discord.ui.UserSelect):
    def __init__(self, canal: discord.VoiceChannel, action: str):
        ph = 'Selecione quem bloquear...' if action == 'block' else 'Selecione quem desbloquear...'
        super().__init__(placeholder=ph, min_values=1, max_values=1)
        self.canal = canal
        self.action = action
...
class KickUserSelect(discord.ui.UserSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if member.voice and member.voice.channel and member.voice.channel.id == self.canal.id:
            await member.move_to(None)
            await interaction.response.send_message(
                f'👢 **{member.display_name}** foi expulso!', ephemeral=True
            )
        else:
            await interaction.response.send_message(
...
class TransferUserSelect(discord.ui.UserSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f'🔄 Canal transferido para **{member.mention}**!', ephemeral=True
        )
...
class RegionSelect(discord.ui.Select):
    def __init__(self, canal: discord.VoiceChannel):
        options = [
            discord.SelectOption(label='🌐 Automático',    value='auto'),
...
# ── INTERFACE VIEW (PERSISTENTE) — 15 BOTÕES SÓ EMOJI ────────────
...
class InterfaceView(discord.ui.View):
    ...
    async def _get_canal(self, interaction: discord.Interaction):
        ...
        if not canal:
            await interaction.response.send_message(
                '❌ Você não tem canal ativo! Entre em **➕ Entrar para Criar** primeiro.',
                ephemeral=True
            )
        return canal

    # ── ROW 0 ──────────────────────────────────────────
    @discord.ui.button(emoji='✏️', style=discord.ButtonStyle.secondary, custom_id='tv_btn_nome', row=0)
    ...
    @discord.ui.button(emoji='👥', style=discord.ButtonStyle.secondary, custom_id='tv_btn_limite', row=0)
    ...
    @discord.ui.button(emoji='🔒', style=discord.ButtonStyle.secondary, custom_id='tv_btn_privacy', row=0)
    async def btn_privacy(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if canal:
            ...
            if pair[1].connect:  # connect está negado = canal bloqueado
                new_ow.connect = None
                await canal.edit(overwrites={**overwrites, everyone: new_ow})
                await interaction.response.send_message('🔓 Canal **aberto**! Todos podem entrar.', ephemeral=True)
            else:
                new_ow.connect = False
                await canal.edit(overwrites={**overwrites, everyone: new_ow})
                await interaction.response.send_message('🔒 Canal **bloqueado**! Só convidados entram.', ephemeral=True)

    @discord.ui.button(emoji='⏳', style=discord.ButtonStyle.secondary, custom_id='tv_btn_waiting', row=0)
        ...
        if canal:
            if canal.id in waiting_rooms:
                ...
                await interaction.response.send_message(
                    ...
                    'Membros aguardarão lá até você usar **📨** para convidá-los.',
                    ephemeral=True
                )

    @discord.ui.button(emoji='💬', style=discord.ButtonStyle.secondary, custom_id='tv_btn_chat', row=0)
    async def btn_chat(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if canal:
            ...
            pair = current.pair()
            new_ow = discord.PermissionOverwrite.from_pair(pair[0], pair[1])
            if pair[1].send_messages:  # send_messages negado
    ...
    # ── ROW 1 ──────────────────────────────────────────
    @discord.ui.button(emoji='🤝', style=discord.ButtonStyle.success, custom_id='tv_btn_trust', row=1)
    async def btn_trust(self, interaction: discord.Interaction, button: discord.ui.Button):
        canal = await self._get_canal(interaction)
        if canal:
            await interaction.response.send_message(
                '🤝 Selecione quem confiar (entra mesmo com canal bloqueado):',
                view=TrustView(canal, 'trust'), ephemeral=True
            )

    @discord.ui.button(emoji='🚫', style=discord.ButtonStyle.secondary, custom_id='tv_btn_untrust', row=1)
    ...
    @discord.ui.button(emoji='📨', style=discord.ButtonStyle.secondary, custom_id='tv_btn_invite', row=1)
    async def btn_invite(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if canal:
            ...
            await interaction.response.send_message(
                f'📨 **Link de convite** (1h / 10 usos):\n{invite.url}', ephemeral=True
            )

    @discord.ui.button(emoji='👢', style=discord.ButtonStyle.danger, custom_id='tv_btn_kick', row=1)
    async def btn_kick(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if canal:
            ...
            if not others:
                await interaction.response.send_message('❌ Ninguém mais no canal.', ephemeral=True)
                return
            await interaction.response.send_message(
                '👢 Selecione quem expulsar:', view=KickSelectView(canal), ephemeral=True
            )

    @discord.ui.button(emoji='🌍', style=discord.ButtonStyle.secondary, custom_id='tv_btn_region', row=1)
    ...
    # ── ROW 2 ──────────────────────────────────────────
    @discord.ui.button(emoji='🚷', style=discord.ButtonStyle.danger, custom_id='tv_btn_block', row=2)
    async def btn_block(self, interaction: discord.Interaction, button: discord.ui.Button):
        canal = await self._get_canal(interaction)
        if canal:
            await interaction.response.send_message(
                '🚷 Selecione quem bloquear:', view=BlockView(canal, 'block'), ephemeral=True
            )

    @discord.ui.button(emoji='✅', style=discord.ButtonStyle.success, custom_id='tv_btn_unblock', row=2)
    ...
    @discord.ui.button(emoji='👑', style=discord.ButtonStyle.secondary, custom_id='tv_btn_claim', row=2)
    async def btn_claim(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for ch_id, owner_id in list(temp_channels.items()):
            if owner_id == interaction.user.id:
                await interaction.response.send_message('❌ Você já é dono de um canal!', ephemeral=True)
                return
            ch = interaction.guild.get_channel(ch_id)
            if not ch or interaction.user not in ch.members:
                continue
        ...
        if claimed:
            ...
            await interaction.response.send_message(
                '❌ Nenhum canal disponível para reivindicar.\n'
                'O dono ainda está presente ou você não está em nenhum canal temporário.',
                ephemeral=True
            )

    @discord.ui.button(emoji='🔄', style=discord.ButtonStyle.secondary, custom_id='tv_btn_transfer', row=2)
    async def btn_transfer(self, interaction: discord.Interaction, button: discord.ui.Button):
        canal = await self._get_canal(interaction)
        if canal:
            await interaction.response.send_message(
                '🔄 Selecione o novo dono:', view=TransferView(canal), ephemeral=True
            )

    @discord.ui.button(emoji='🗑️', style=discord.ButtonStyle.danger, custom_id='tv_btn_delete', row=2)
    ...
    async def _handle_vote(self, interaction: discord.Interaction, vote_type: str):
        try:
            ...
            if previous == vote_type:
                await interaction.response.send_message(
                    '⚠️ Você já votou nessa opção! Clique no outro botão para trocar.',
                    ephemeral=True
                )
                return
...
class TDNBot(commands.Bot):
    ...
    async def on_voice_state_update(
        ...
        try:
            # 1) Entrou no canal "Entrar para Criar" → cria canal temporário
            if after.channel and after.channel.id == JOIN_TO_CREATE_ID:
                ...
                if iface_channel:
                    embed = discord.Embed(
                        title='🎙️ Interface do Canal',
                        description=(
                            'Use os botões abaixo para gerenciar seu canal de voz.\n'
                            '> Somente o **dono** do canal pode usar os botões.'
                        ),
                        color=0x5865F2
                    )
                    embed.add_field(name='🎤 Canal', value=f'`{novo_canal.name}`', inline=True)
                    embed.add_field(name='👑 Dono', value=member.mention, inline=True)
                    embed.add_field(
                        name='Legenda dos botões',
                        value=(
                            '✏️ Nome · 👥 Limite · 🔒 Privacidade · ⏳ Sala Espera · 💬 Chat\n'
                            '🤝 Confiar · 🚫 Desconfiar · 📨 Convidar · 👢 Kick · 🌍 Região\n'
                            '🚷 Bloquear · ✅ Desbloquear · 👑 Reivindicar · 🔄 Transferir · 🗑️ Deletar'
                        ),
                        inline=False
                    )
                ...
                logger.info(f'Canal temp criado: {novo_canal.name} ({novo_canal.id}) para {member}')

            # 2) Alguém entra em canal temporário com sala de espera ativa
            if (after.channel and after.channel.id in temp_channels
                    and after.channel.id in waiting_rooms):
                ch = after.channel
                owner_id = temp_channels.get(ch.id)
                if member.id != owner_id:
                    ow = ch.overwrites_for(member)
                    if ow.connect is not True:
                        wr_ch = member.guild.get_channel(waiting_rooms[ch.id])
                        if wr_ch:
                            await member.move_to(wr_ch)
                            logger.info(f'{member} movido para sala de espera de {ch.name}')

            # 3) Saiu de canal temporário → apaga se vazio
